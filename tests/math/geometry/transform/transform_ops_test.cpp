...
TEST_CASE("Transform - Binary Multiplication Identity", "[transform][ops][multiply]") {
    Transform<> T1 = Transform<>::translationVector(Vector<3>{1.0f, 2.0f, 3.0f});
...
TEST_CASE("Transform - Binary Multiplication Chaining Three Transforms", "[transform][ops][multiply]") {
    Transform<> T1 = Transform<>::rotationX(M_PI / 2.0f);
    Transform<> T2 = Transform<>::translationVector(Vector<3>{1.0f, 0.0f, 0.0f});
    Transform<> T3 = Transform<>::rotationZ(M_PI / 2.0f);
...
TEST_CASE("Transform - Binary Multiplication Associativity", "[transform][ops][multiply]") {
    Transform<> T1 = Transform<>::rotationX(0.3f);
    Transform<> T2 = Transform<>::translationVector(Vector<3>{1.0f, 2.0f, 3.0f});
    Transform<> T3 = Transform<>::rotationZ(0.5f);
...
TEST_CASE("Transform - Vector Multiplication Translation", "[transform][ops][vector]") {
    Transform<> T = Transform<>::translationVector(Vector<3>{5.0f, 10.0f, 15.0f});
    Vector<3> v = {1.0f, 2.0f, 3.0f};
...
TEST_CASE("Transform - Vector Multiplication Combined", "[transform][ops][vector]") {
    ...
    Quaternion quat = Quaternion::axisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, M_PI / 2.0f);
    Transform<> T = Transform<>::pose(trans, quat);
...
TEST_CASE("Transform - Vector Multiplication Multiple Points", "[transform][ops][vector]") {
    Transform<> T = Transform<>::rotationZ(M_PI / 4.0f);
...
TEST_CASE("Transform - Equality Same Translation", "[transform][ops][equality]") {
    ...
    Transform<> T1 = Transform<>::translationVector(t);
    Transform<> T2 = Transform<>::translationVector(t);
    
    REQUIRE(T1 == T2);
}

TEST_CASE("Transform - Equality Same Rotation", "[transform][ops][equality]") {
    Transform<> T1 = Transform<>::rotationZ(M_PI / 3.0f);
    Transform<> T2 = Transform<>::rotationZ(M_PI / 3.0f);
...
TEST_CASE("Transform - Equality Different Translation", "[transform][ops][equality]") {
    Transform<> T1 = Transform<>::translationVector(Vector<3>{1.0f, 2.0f, 3.0f});
    Transform<> T2 = Transform<>::translationVector(Vector<3>{1.0f, 2.0f, 4.0f});
...
TEST_CASE("Transform - Inverse Composition T * T^-1 = I", "[transform][ops][inverse]") {
    Vector<3> t = {1.0f, 2.0f, 3.0f};
    Quaternion q = Quaternion::axisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, M_PI / 3.0f);
    Transform<> T = Transform<>::pose(t, q);
    Transform<> T_inv = inv(T);
...
TEST_CASE("Transform - Inverse Composition T^-1 * T = I", "[transform][ops][inverse]") {
    ...
    Transform<> T = Transform<>::pose(t, Quaternion::axisAngle(Vector<3>{1.0f, 1.0f, 0.0f}, 0.7f));
    Transform<> T_inv = inv(T);
...
TEST_CASE("Transform - Double Inverse", "[transform][ops][inverse]") {
    Transform<> T = Transform<>::pose(Vector<3>{1.0f, 2.0f, 3.0f},
                                          Quaternion::axisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, M_PI / 4.0f));
...
TEST_CASE("Transform - Inverse of Composition (AB)^-1 = B^-1 A^-1", "[transform][ops][inverse]") {
    Transform<> A = Transform<>::rotationZ(M_PI / 6.0f);
    Transform<> B = Transform<>::translationVector(Vector<3>{1.0f, 2.0f, 0.0f});
...
TEST_CASE("Transform - Robot Chain Forward-Backward", "[transform][integration]") {
    ...
    Transform<> base_to_link1 = Transform<>::rotationZ(M_PI / 4.0f);
    Transform<> link1_to_link2 = Transform<>::translationVector(Vector<3>{1.0f, 0.0f, 0.0f});
    Transform<> link2_to_ee = Transform<>::rotationZ(M_PI / 4.0f);
...
TEST_CASE("Transform - Sensor Frame Change", "[transform][integration]") {
    ...
    Transform<> world_to_base = Transform<>::pose(
        Vector<3>{0.0f, 0.0f, 0.5f},
        Quaternion::eye()
    );
    
    // Robot base to sensor
    Transform<> base_to_sensor = Transform<>::pose(
        Vector<3>{0.2f, 0.0f, 0.1f},
        Quaternion::axisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, M_PI / 2.0f)
    );
...
TEST_CASE("Transform - Relative Transform Between Frames", "[transform][integration]") {
    // Frame A
    Transform<> world_to_A = Transform<>::pose(
        Vector<3>{1.0f, 0.0f, 0.0f},
        Quaternion::axisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, M_PI / 4.0f)
    );
    
    // Frame B
    Transform<> world_to_B = Transform<>::pose(
