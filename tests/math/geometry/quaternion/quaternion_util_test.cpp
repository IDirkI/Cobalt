...
TEST_CASE("Quaternion - isNormalized True", "[quaternion][util][check]") {
    Quaternion<> q = Quaternion<>::fromAxisAngle(Vector<3>{1.0f, 0.0f, 0.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - isPure True", "[quaternion][util][check]") {
    Quaternion<> q = Quaternion<>::fromPureVector(Vector<3>{1.0f, 2.0f, 3.0f});
...
TEST_CASE("Quaternion - isSameRotation Identical", "[quaternion][util][check]") {
    Quaternion<> q1 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 3.0f);
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 3.0f);
...
TEST_CASE("Quaternion - isSameRotation Different", "[quaternion][util][check]") {
    Quaternion<> q1 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 3.0f);
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - toVector Round-Trip", "[quaternion][util][convert]") {
    Vector<3> origVec = {0.0f, float(M_PI) / 3.0f, 0.0f};
...
TEST_CASE("Quaternion - toAngle 90 Degrees", "[quaternion][util][convert]") {
    Quaternion<> q = Quaternion<>::fromAxisAngle(Vector<3>{1.0f, 0.0f, 0.0f}, float(M_PI) / 2.0f);
    
    float angle = toAngle(q);
    
    REQUIRE_THAT(angle, Catch::Matchers::WithinAbs(float(M_PI) / 2.0f, 1e-4f));
}

TEST_CASE("Quaternion - toAxis X-Axis", "[quaternion][util][convert]") {
    Quaternion<> q = Quaternion<>::fromAxisAngle(Vector<3>{1.0f, 0.0f, 0.0f}, float(M_PI) / 3.0f);
...
TEST_CASE("Quaternion - angledDistance Same Rotation", "[quaternion][util][distance]") {
    Quaternion<> q1 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 4.0f);
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - angledDistance 90 Degrees", "[quaternion][util][distance]") {
    ...
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, float(M_PI) / 2.0f);
    
    float dist = angledDistance(q1, q2);
    
    REQUIRE_THAT(dist, Catch::Matchers::WithinAbs(float(M_PI) / 2.0f, 1e-4f));
}
...
TEST_CASE("Quaternion - swingTwist Z-Axis", "[quaternion][util][rotate]") {
    ...
    Quaternion<> q = Quaternion<>::fromAxisAngle(Vector<3>{1.0f, 1.0f, 1.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - Angular Distance Symmetry", "[quaternion][integration]") {
    Quaternion<> q1 = Quaternion<>::fromAxisAngle(Vector<3>{1.0f, 0.0f, 0.0f}, float(M_PI) / 6.0f);
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - Difference Composition", "[quaternion][integration]") {
    Quaternion<> q1 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 6.0f);
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 3.0f);
...
TEST_CASE("Quaternion - SLERP Midpoint", "[quaternion][util][interp]") {
    ...
    Quaternion<> expected = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - SLERP Handles Opposite Quaternions", "[quaternion][util][interp]") {
    Quaternion<> q1 = Quaternion<>::eye();
    Quaternion<> q2 = -Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - NLERP Similar to SLERP for Small Angles", "[quaternion][util][interp]") {
    Quaternion<> q1 = Quaternion<>::eye();
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 8.0f);
...
TEST_CASE("Quaternion - NLERP Handles Opposite Sign", "[quaternion][util][interp]") {
    Quaternion<> q1 = Quaternion<>::eye();
    Quaternion<> q2 = -Quaternion<>::fromAxisAngle(Vector<3>{1.0f, 0.0f, 0.0f}, float(M_PI) / 4.0f);
...
TEST_CASE("Quaternion - shortestPath Preserves Magnitude", "[quaternion][util][path]") {
    Quaternion<> q1 = Quaternion<>::eye();
    Quaternion<> q2 = Quaternion<>::fromAxisAngle(Vector<3>{1.0f, 1.0f, 1.0f}, float(M_PI));
...
TEST_CASE("Quaternion - shortestPath Critical for Control", "[quaternion][util][path]") {
    ...
    Quaternion<> current = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) - 0.1f);
    Quaternion<> target = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, -float(M_PI) + 0.1f);
...
TEST_CASE("Quaternion - clampRotation Within Limit", "[quaternion][util][clamp]") {
    Quaternion<> q = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, float(M_PI) / 6.0f);
    
    Quaternion<> result = clampRotation(q, float(M_PI) / 3.0f);
...
TEST_CASE("Quaternion - clampRotation Exceeds Limit", "[quaternion][util][clamp]") {
    ...
    float maxAngle = float(M_PI) / 2.0f;
...
TEST_CASE("Quaternion - clampRotation Preserves Axis", "[quaternion][util][clamp]") {
    ...
    Quaternion<> result = clampRotation(q, float(M_PI)/4.0f);
...
TEST_CASE("Quaternion - clampRotation Unit Quaternion<> Output", "[quaternion][util][clamp]") {
    Quaternion<> q = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 1.0f, 0.0f}, 2.0f * float(M_PI) / 3.0f);
    
    Quaternion<> result = clampRotation(q, float(M_PI)/6.0f);
...
TEST_CASE("Quaternion - clampRotation Large Limit", "[quaternion][util][clamp]") {
    ...
    Quaternion<> result = clampRotation(q, 10.0f * float(M_PI));
...
TEST_CASE("Quaternion - clampRotation Rate Limiting Use Case", "[quaternion][util][clamp]") {
    ...
    float maxRatePerSec = float(M_PI) / 4.0f;  // 45°/sec
    float dt = 0.1f;  // 100ms
    float maxAngle = maxRatePerSec * dt;  // 4.5°
    
    Quaternion<> desired = Quaternion<>::fromAxisAngle(Vector<3>{0.0f, 0.0f, 1.0f}, float(M_PI) / 2.0f);
