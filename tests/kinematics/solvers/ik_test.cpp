...
static void applyAndLog(KukaRobot &robot, KukaFK &fk,
    ...
    KukaLog::snapshot(robot, filename);
    printf("  logged     : %s.clog\n", filename);
}
...
// ============================================================================
//  Tests - identical to before except logging calls
// ============================================================================
...
TEST_CASE("InverseKinematics, arc trajectory with per-waypoint logging", "[ik][kinematics]") {
    ...
    // One session file - all waypoints as frames - enables animation in plotter
    KukaLog session("ik_trajectory", robot);
    ...
    for(int wp = 0; wp < nWaypoints; wp++) {
        ...
        // Log as frame in session - timestamp is waypoint index in microseconds
        session.log(robot, static_cast<uint64_t>(wp) * 1000000ULL);
    ...
    // session destructor flushes and closes ik_trajectory.clog here
    printf("\n  Logged all waypoints to: ik_trajectory.clog\n");
}

TEST_CASE("InverseKinematics, circular trajectory closes and converges", "[ik][kinematics]") {
    KukaRobot robot = makeKuka();
    KukaFK fk(robot);
    ...
    constexpr float cz         = 1.5f;
    constexpr float r          = 0.2f;
    constexpr int   nWaypoints = 24;

    robot.setJoints(Vector<6>{ 0.0f, -0.5f, 0.8f, 0.0f, 0.0f, 0.0f });
    fk.solve(robot.state());

    printf("\n[Circular trajectory - %d waypoints, center=[%.1f,%.1f,%.1f] r=%.2f]\n",
           nWaypoints, cx, cy, cz, r);
    printf("  wp   status        iters   pos_err(m)   x        y        z\n");
    printf("  --   ------        -----   ----------   ------   ------   ------\n");

    // One session file - all waypoints + closing frame for animation
    KukaLog session("ik_circle", robot);

    // Store first achieved position to verify closure against
    Vector<3> p_first = Vector<3>::zero();

    for(int wp = 0; wp < nWaypoints; wp++) {
        const float angle = (float)wp / (float)nWaypoints * 2.0f * (float)M_PI;
        ...
        // Timestamp = angle in microradians so the plotter shows smooth animation
        session.log(robot, static_cast<uint64_t>(angle * 1e6f));
    ...
    IKSolution<6> sol_close = ik.solve(closing);

    robot.setJoints(sol_close.q);
    fk.solve(robot.state());

    session.log(robot, static_cast<uint64_t>(2.0f * (float)M_PI * 1e6f));

    const Vector<3> &p_closing = robot.state().frameTransforms[0].translation();
    const float      closeErr  = cobalt::math::linear_algebra::norm(p_closing - p_first);

    printf("\n  Closing frame: iters=%d  pos_err=%.6f\n",
           sol_close.iterations, positionError(sol_close));
    printf("  Circle closure error: %.6f m\n", closeErr);
    printf("    wp0     : [ %.4f  %.4f  %.4f ]\n", p_first[0],   p_first[1],   p_first[2]);
    printf("    closing : [ %.4f  %.4f  %.4f ]\n", p_closing[0], p_closing[1], p_closing[2]);

    REQUIRE((sol_close.status == IKStatus::Success || sol_close.status == IKStatus::MaxIterations));
    REQUIRE_THAT(positionError(sol_close), Catch::Matchers::WithinAbs(0.0f, 5e-3f));
    REQUIRE_THAT(closeErr, Catch::Matchers::WithinAbs(0.0f, 5e-3f));

    printf("\n  Logged: ik_circle.clog\n");
    printf("  Visualise: python scripts/kinematics/robot_plotter.py ik_circle\n");
}